...
export const AdminModules: React.FC<AdminModulesProps> = ({ currentTab, addToast, onRefreshStats }) => {
  ...
  const teacherStats = useMemo(() => {
    ...
    return teachers.map(teacher => {
      const tAssigns = assignments.filter(asg => asg.teacher_id === teacher.id);
      
      let totalTpsCount = 0;
      let totalScopesCount = 0;
      
      let totalExpectedFormative = 0;
      let totalActualFormative = 0;
      
      let totalExpectedSummative = 0;
      let totalActualSummative = 0;
      
      let totalExpectedSemester = 0;
      let totalActualSemester = 0;

      const assignmentsDetails = tAssigns.map(asg => {
        const clsName = classes.find(c => c.id === asg.class_id)?.nama || 'Kelas';
        const subName = subjects.find(s => s.id === asg.subject_id)?.nama || 'Mata Pelajaran';
        
        // Count TPs
        const asgTps = tps.filter(tp => tp.class_id === asg.class_id && tp.subject_id === asg.subject_id);
        const tpsCount = asgTps.length;
        totalTpsCount += tpsCount;
        
        // Count Scopes
        const asgScopes = scopes.filter(sc => sc.class_id === asg.class_id && sc.subject_id === asg.subject_id);
        const scopesCount = asgScopes.length;
        totalScopesCount += scopesCount;
        
        // Students in this class
        const classStudents = students.filter(s => s.kelas_id === asg.class_id);
        const studentsCount = classStudents.length;
        
        // Expected Formative
        const expFormative = studentsCount * tpsCount;
        totalExpectedFormative += expFormative;
        
        // Actual Formative
        const tpIds = new Set(asgTps.map(t => t.id));
        const studentIds = new Set(classStudents.map(s => s.id));
        const actFormative = formativeScores.filter(fs => 
          studentIds.has(fs.student_id) && 
          tpIds.has(fs.tp_id) &&
          fs.semester === semId &&
          fs.tahun === yrId
        ).length;
        totalActualFormative += actFormative;
        
        // Expected Summative
        const expSummative = studentsCount * scopesCount;
        totalExpectedSummative += expSummative;
        
        // Actual Summative
        const scopeIds = new Set(asgScopes.map(sc => sc.id));
        const actSummative = summativeScores.filter(ss => 
          studentIds.has(ss.student_id) && 
          scopeIds.has(ss.lingkup_id) &&
          ss.semester === semId &&
          ss.tahun === yrId
        ).length;
        totalActualSummative += actSummative;
        
        // Expected Semester
        const expSemester = studentsCount;
        totalExpectedSemester += expSemester;
        
        // Actual Semester
        const actSemester = semesterScores.filter(ses => 
          studentIds.has(ses.student_id) && 
          ses.subject_id === asg.subject_id &&
          ses.semester === semId &&
          ses.tahun === yrId
        ).length;
        totalActualSemester += actSemester;
        
        const subTotalExpected = expFormative + expSummative + expSemester;
        const subTotalActual = actFormative + actSummative + actSemester;
        const subPercentage = subTotalExpected > 0 ? Math.round((subTotalActual / subTotalExpected) * 100) : 0;
        ...
        return {
          ...
          tpsCount,
          scopesCount,
          actualFormative: actFormative,
          expectedFormative: expFormative,
          actualSummative: actSummative,
          expectedSummative: expSummative,
          actualSemester: actSemester,
          expectedSemester: expSemester,
          subPercentage
        };
      });
      
      const grandExpected = totalExpectedFormative + totalExpectedSummative + totalExpectedSemester;
      const grandActual = totalActualFormative + totalActualSummative + totalActualSemester;
      const overallPercentage = grandExpected > 0 ? Math.round((grandActual / grandExpected) * 100) : 0;
      
      return {
        teacher,
        assignmentsCount: tAssigns.length,
        totalTpsCount,
        totalScopesCount,
        overallPercentage,
        assignmentsDetails,
  ...
  }, [teachers, assignments, tps, scopes, formativeScores, summativeScores, semesterScores, students, classes, subjects, context]);
      ...
      {currentTab === 'logs' && (
        <div className="space-y-6">
          <div className="bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-sm">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 border-b border-slate-100 bg-slate-50/50">
              <div>
                <h3 className="text-base font-bold text-slate-800">
                  {showDetailedLogs ? "Log Riwayat Aktivitas Guru" : "Monitoring Pengisian Data Guru"}
                </h3>
                <p className="text-xs text-slate-500 mt-1">
                  {showDetailedLogs 
                    ? "Log riwayat kejadian audit pengisian TP, lingkup materi, dan penilaian nilai siswa secara rinci."
                    : "Monitoring jumlah pengisian Tujuan Pembelajaran (TP), Lingkup Materi (LM), dan persentase pengisian nilai siswa per guru."}
                </p>
              </div>
              <div className="flex items-center gap-2 flex-wrap">
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl text-xs font-bold border transition-all flex items-center gap-1.5 bg-white border-slate-200 text-slate-700 hover:bg-slate-50 shadow-sm"
                >
                  <History className="w-3.5 h-3.5 text-slate-500" />
                  {showDetailedLogs ? "Lihat Ringkasan Progres" : "Lihat Log Audit Detail"}
                </button>
              </div>
            </div>

            {/* --- VIEW 1: TEACHER MONITORING STATS (DEFAULT) --- */}
            {!showDetailedLogs && (
              <div className="p-6 space-y-6">
                ...
                <div className="overflow-x-auto border border-slate-100 rounded-xl">
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-100">
                        <th className="px-6 py-4 text-xs font-bold uppercase text-slate-400 tracking-wider">Nama Guru / NIP</th>
                        <th className="px-6 py-4 text-xs font-bold uppercase text-slate-400 tracking-wider text-center">Kelas & Mapel</th>
                        <th className="px-6 py-4 text-xs font-bold uppercase text-slate-400 tracking-wider text-center">Jumlah TP</th>
                        <th className="px-6 py-4 text-xs font-bold uppercase text-slate-400 tracking-wider text-center">Jumlah Lingkup Materi</th>
                        <th className="px-6 py-4 text-xs font-bold uppercase text-slate-400 tracking-wider">Persentase Pengisian Nilai</th>
                        <th className="px-6 py-4 text-xs font-bold uppercase text-slate-400 tracking-wider text-center w-24">Detail</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {teacherStats
                        .filter(stat => 
                          stat.teacher.nama.toLowerCase().includes(searchTeacherQuery.toLowerCase()) || 
                          (stat.teacher.nip && stat.teacher.nip.includes(searchTeacherQuery))
                        )
                        .length === 0 ? (
                        <tr>
                          <td colSpan={6} className="px-6 py-16 text-center text-slate-400">
                            <div className="flex flex-col items-center justify-center max-w-sm mx-auto">
                              ...
                              <p className="text-sm font-bold text-slate-800">Tidak ada guru ditemukan</p>
                        ...
                        teacherStats
                          ...
                          .map((stat) => {
                            const isExpanded = expandedTeachers.includes(stat.teacher.id);
                            
                            // Color code progress bar
                            let progressColor = 'bg-slate-400';
                            let textColor = 'text-slate-600';
                            let badgeColor = 'bg-slate-50 border-slate-100';

                            if (stat.assignmentsCount === 0) {
                              progressColor = 'bg-slate-200';
                              textColor = 'text-slate-400';
                              badgeColor = 'bg-slate-50 border-slate-100 text-slate-400';
                            } else if (stat.overallPercentage === 100) {
                              progressColor = 'bg-emerald-500';
                              textColor = 'text-emerald-700 font-bold';
                              badgeColor = 'bg-emerald-50 text-emerald-700 border-emerald-100';
                            } else if (stat.overallPercentage >= 80) {
                              progressColor = 'bg-teal-500';
                              textColor = 'text-teal-700 font-bold';
                              badgeColor = 'bg-teal-50 text-teal-700 border-teal-100';
                            } else if (stat.overallPercentage > 0) {
                              ...
                              <React.Fragment key={stat.teacher.id}>
                                <tr className="hover:bg-slate-50/50 transition-colors">
                                  <td className="px-6 py-4">
                                    <div className="flex items-center gap-3">
                                      <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center font-bold text-slate-600 text-xs uppercase border border-slate-200">
                                        {stat.teacher.nama.substring(0, 2)}
                                      </div>
                                      <div>
                                        <div className="text-slate-800 font-bold text-sm">{stat.teacher.nama}</div>
                                        {stat.teacher.nip && (
                                          <div className="text-slate-400 text-[11px] font-mono mt-0.5">NIP. {stat.teacher.nip}</div>
                                        )}
                                      </div>
                                    </div>
                                  </td>
                                  <td className="px-6 py-4 text-center text-slate-600 text-xs font-semibold">
                                    {stat.assignmentsCount === 0 ? (
                                      <span className="text-slate-400 font-normal italic">Belum ditugaskan</span>
                                    ) : (
                                      <span className="bg-slate-100 text-slate-700 px-2.5 py-1 rounded-lg">
                                        {stat.assignmentsCount} Tugas
                                      </span>
                                    )}
                                  </td>
                                  <td className="px-6 py-4 text-center">
                                    <span className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-semibold rounded-lg bg-indigo-50 text-indigo-700 border border-indigo-100">
                                      <BookOpen className="w-3.5 h-3.5" />
                                      {stat.totalTpsCount} TP
                                    </span>
                                  </td>
                                  <td className="px-6 py-4 text-center">
                                    <span className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-semibold rounded-lg bg-amber-50 text-amber-700 border border-amber-100">
                                      <Layers className="w-3.5 h-3.5 text-amber-600" />
                                      {stat.totalScopesCount} LM
                                    </span>
                                  </td>
                                  <td className="px-6 py-4">
                                    {stat.assignmentsCount === 0 ? (
                                      ...
                                      <div className="space-y-1.5 max-w-xs">
                                        <div className="flex items-center justify-between">
                                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${badgeColor}`}>
                                            {stat.overallPercentage}% Terisi
                                          </span>
                                          <span className="text-[10px] text-slate-400 font-semibold">
                                            {stat.grandActual} / {stat.grandExpected} Sel
                                          </span>
                                        </div>
                                        <div className="w-full bg-slate-100 rounded-full h-2">
                                          <div 
                                            className={`h-2 rounded-full transition-all duration-500 ${progressColor}`}
                                            style={{ width: `${stat.overallPercentage}%` }}
                                          />
                                        </div>
                                      </div>
                                    )}
                                  ...
                                  <td className="px-6 py-4 text-center">
                                    <button
                                      type="button"
                                      disabled={stat.assignmentsCount === 0}
                                      onClick={() => {
                                        if (isExpanded) {
                                          setExpandedTeachers(expandedTeachers.filter(id => id !== stat.teacher.id));
                                        } else {
                                          setExpandedTeachers([...expandedTeachers, stat.teacher.id]);
                                        }
                                      }}
                                      className="p-1.5 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-slate-600 transition-all cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed"
                                    >
                                      {isExpanded ? (
                                ...
                                {/* Expanded detailed stats for this teacher */}
                                {isExpanded && stat.assignmentsCount > 0 && (
                                  <tr>
                                    <td colSpan={6} className="px-6 py-4 bg-slate-50/70 border-y border-slate-100">
                                      <div className="p-4 bg-white border border-slate-100 rounded-xl space-y-3 shadow-inner">
                                        <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                                          <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider">
                                            Rincian Pengisian Data Nilai: {stat.teacher.nama}
                                          </h4>
                                          <span className="text-[10px] text-slate-400 font-semibold">
                                            Tahun: {context.year?.nama || '2025'} | Semester: {context.semester?.nama || 'Ganjil'}
                                          </span>
                                        </div>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                          {stat.assignmentsDetails.map((asgDetail, idx) => (
                                            <div key={idx} className="border border-slate-100 rounded-lg p-3 bg-slate-50/30 space-y-2">
                                              <div className="flex items-center justify-between">
                                                <div className="font-bold text-xs text-slate-800">
                                                  {asgDetail.className} - {asgDetail.subjectName}
                                                </div>
                                                <div className="text-[10px] font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-full">
                                                  {asgDetail.subPercentage}% Terisi
                                                </div>
                                              </div>

                                              <div className="grid grid-cols-3 gap-2 text-[11px] text-slate-500 pt-1 border-t border-slate-100/50">
                                                <div>
                                                  <div className="font-semibold text-slate-400">Nilai Formatif</div>
                                                  <div className="mt-0.5 font-bold text-slate-700">
                                                    {asgDetail.actualFormative}/{asgDetail.expectedFormative}
                                                  </div>
                                                  <div className="text-[9px] text-slate-450">
                                                    ({asgDetail.tpsCount} TP × {asgDetail.studentsCount} siswa)
                                                  </div>
                                                </div>
                                                <div>
                                                  <div className="font-semibold text-slate-400">Nilai Sumatif LM</div>
                                                  <div className="mt-0.5 font-bold text-slate-700">
                                                    {asgDetail.actualSummative}/{asgDetail.expectedSummative}
                                                  </div>
                                                  <div className="text-[9px] text-slate-450">
                                                    ({asgDetail.scopesCount} LM × {asgDetail.studentsCount} siswa)
                                                  </div>
                                                </div>
                                                <div>
                                                  <div className="font-semibold text-slate-400">Nilai Semester</div>
                                                  <div className="mt-0.5 font-bold text-slate-700">
                                                    {asgDetail.actualSemester}/{asgDetail.expectedSemester}
                                                  </div>
                                                  <div className="text-[9px] text-slate-450">
                                                    (1 Mapel × {asgDetail.studentsCount} siswa)
                                                  </div>
                                                </div>
                                              </div>
                                              ...
                                              <div className="w-full bg-slate-100 rounded-full h-1 mt-1.5">
                                                <div 
                                                  ...
                                                  style={{ width: `${asgDetail.subPercentage}%` }}
                                                />
                                              </div>
                                            </div>
                                          ))}
            ...
            {/* --- VIEW 2: ORIGINAL AUDIT LOGS (DETAILED) --- */}
            {showDetailedLogs && (
              <>
                <div className="flex items-center gap-2 flex-wrap p-6 border-b border-slate-100 bg-slate-50/30">
                  <span className="text-xs text-slate-500 font-bold mr-2">Filter Kategori:</span>
                  <button
                    type="button"
